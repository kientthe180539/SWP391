            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if ("ROUTINE".equals(typeStr))
                        typeStr = "INSPECTION";
                    try {
                        t.setTaskType(HousekeepingTask.TaskType.valueOf(typeStr));
                    } catch (IllegalArgumentException e) {
                        t.setTaskType(HousekeepingTask.TaskType.INSPECTION); // Fallback
                    }

                    t.setStatus(HousekeepingTask.TaskStatus.DONE);
