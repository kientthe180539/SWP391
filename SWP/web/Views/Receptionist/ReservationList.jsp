                    ...
                    <div class="container">
                            ...
                            <form method="GET" action="${pageContext.request.contextPath}/receptionist/reservations">
                                <div class="filter-grid">
                                    <select name="status">
                                        ...
                                        <option value="CHECKED_IN" ${status=='CHECKED_IN' ? 'selected' : '' }>Checked In
                                        </option>
                                        <option value="CHECKED_OUT" ${status=='CHECKED_OUT' ? 'selected' : '' }>Checked
                                            Out</option>
                                        <option value="CANCELLED" ${status=='CANCELLED' ? 'selected' : '' }>Cancelled
                                        </option>
                                    </select>
                                    <select name="sortBy">
                                        <option value="booking_date" ${sortBy=='booking_date' ? 'selected' : '' }>Sort
                                            by Date
                                        </option>
                                        <option value="total_amount" ${sortBy=='total_amount' ? 'selected' : '' }>Sort
                                            by Amount
                                        </option>
                                        <option value="customer_name" ${sortBy=='customer_name' ? 'selected' : '' }>Sort
                                            by Name
                                        </option>
                                    </select>
                        ...
                        <div class="table-section">
                            <p style="margin-bottom: 20px; color: #777; font-size: 15px;">
                                ...
                                Page <strong style="color: #1e3c72;">${currentPage}</strong>/<strong
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="${pageContext.request.contextPath}/receptionist/reservation-detail?id=${booking.bookingId}"
                                                        style="color: #2a5298; text-decoration: none; font-weight: 600;">View
                                                        →</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                ...
                                <div class="pagination">
                                    ...
                                    <c:forEach begin="1" end="${totalPages}" var="i">
                                        <c:if
                                            ...
                                            <a href="?page=${i}&status=${status}&search=${search}&dateFrom=${dateFrom}&dateTo=${dateTo}&sortBy=${sortBy}"
                                    ...
                                    <c:if test="${currentPage < totalPages}">
                                        <a
                                            href="?page=${currentPage + 1}&status=${status}&search=${search}&dateFrom=${dateFrom}&dateTo=${dateTo}&sortBy=${sortBy}">Next
                                            »</a>
                                    </c:if>
                                </div>
                            </c:if>
